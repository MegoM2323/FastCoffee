...
async def cmd_start(message: Message):
    keyboard = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="/colors"), KeyboardButton(text="/help"), KeyboardButton(text="/way")]], resize_keyboard=True)
    if data_handler.add_user(telegram_id=str(message.from_user.id), name=message.from_user.username) == 1:
        await message.answer(Config.hello_text[0], reply_markup=keyboard)
        print(f'+ {message.from_user.id} {message.from_user.username}')
    else:
        await message.answer(Config.hello_text[1], reply_markup=keyboard)
